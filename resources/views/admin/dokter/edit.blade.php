        ...
        <form method="POST" action="{{ route('admin-dokter.update', $dokter->id) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
              ...
              <textarea name="alamat_dokter" class="form-control" id="exampleFormControlTextarea1"
                rows="3">{{ old('alamat_dokter', $dokter->alamat_dokter) }}</textarea>
            </div>
            <div class="form-group">
              ...
              <select name="spesialisasi_dokter" class="form-control" id="spesialisasi_dokter">
                <option value="Poli Umum" {{ old('spesialisasi_dokter', $dokter->spesialisasi_dokter) == 'Poli Umum' ? 'selected' : '' }}>Poli Umum</option>
                <option value="Poli Anak" {{ old('spesialisasi_dokter', $dokter->spesialisasi_dokter) == 'Poli Anak' ? 'selected' : '' }}>Poli Anak</option>
                <option value="Poli Lansia" {{ old('spesialisasi_dokter', $dokter->spesialisasi_dokter) == 'Poli Lansia' ? 'selected' : '' }}>Poli Lansia</option>
              </select>
            </div>
